    ...
    if v_draft is not null then
      ...
      loop
        ...
        values (v_prayer_id, v_turn.user_id, v_ord,
                coalesce(nullif((v_pline.ln)->>'movement','')::public.prayer_movement, 'petition'::public.prayer_movement),
...
REVOKE ALL ON FUNCTION public.persist_unified_turn(uuid, uuid, jsonb, jsonb, text, integer, integer, integer, integer) FROM PUBLIC, anon, authenticated;
